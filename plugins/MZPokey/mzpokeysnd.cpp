...
#define NPOKEYS 1			//f0 from 2 to 1
...
typedef unsigned char (*readout_t)(struct MZPokey* ps);		//f0
typedef void (*event_t)(struct MZPokey* ps, char p5v, char p4v, char p917v);	//f0


/* State variables for single Pokey Chip */
typedef struct MZPokey : public Unit	//f0
{
    /* Poly positions */
...
extern "C" {	//f0
	...
	int Pokey_sound_init_mz(uint32 freq17, uint16 playback_freq, uint8 num_pokeys, int flags, int quality
#ifdef __PLUS
		, int clear_regs
#endif
	);
...
PluginLoad(InterfaceTable *inTable) {	//f0
	ft= inTable;
	DefineSimpleUnit(MZPokey);
	Pokey_sound_init_mz(FREQ_17_APPROX, 44100, NPOKEYS, SND_BIT16, 0	//f0 hardcoded to 44100 - bad
		//but do not know how to access sr here in the load function
#ifdef __PLUS
		, 0
#endif
	);
...
void MZPokey_Ctor(PokeyState *unit) {	//f0
	unit->last0= 0.f;
	unit->last1= 0.f;
	unit->last2= 0.f;
	unit->last3= 0.f;
	...
	unit->last7= 0.f;
	unit->last8= 0.f;
	SETCALC(MZPokey_next_k);
	ResetPokeyState(pokey_states);
	ResetPokeyState(pokey_states + 1);
	MZPokey_next_k(unit, 1);
}

void MZPokey_next_k(PokeyState *unit, int inNumSamples) {	//f0
	float *out= OUT(0);
	if(IN0(0)!=unit->last0) {
		unit->last0= IN0(0);
		Update_pokey_sound(_AUDF1, unit->last0, 0, 12);
	}
	if(IN0(1)!=unit->last1) {
		unit->last1= IN0(1);
		Update_pokey_sound(_AUDC1, unit->last1, 0, 12);
	}
	if(IN0(2)!=unit->last2) {
		unit->last2= IN0(2);
		Update_pokey_sound(_AUDF2, unit->last2, 0, 12);
	}
	if(IN0(3)!=unit->last3) {
		unit->last3= IN0(3);
		Update_pokey_sound(_AUDC2, unit->last3, 0, 12);
	}
	if(IN0(4)!=unit->last4) {
		unit->last4= IN0(4);
		Update_pokey_sound(_AUDF3, unit->last4, 0, 12);
	}
	if(IN0(5)!=unit->last5) {
		unit->last5= IN0(5);
		Update_pokey_sound(_AUDC3, unit->last5, 0, 12);
	}
	if(IN0(6)!=unit->last6) {
		unit->last6= IN0(6);
		Update_pokey_sound(_AUDF4, unit->last6, 0, 12);
	}
	if(IN0(7)!=unit->last7) {
		unit->last7= IN0(7);
		Update_pokey_sound(_AUDC4, unit->last7, 0, 12);
	}
	if(IN0(8)!=unit->last8) {
		unit->last8= IN0(8);
		Update_pokey_sound(_AUDCTL, unit->last8, 0, 12);
	}
	Pokey_process_ptr(out, inNumSamples);							//buffer, n
}
...
found:
  ...
  //remez(ps, filter_data, (size / interlevel) + 1, 2, bands, desired, weights, BANDPASS);
...
static void Pokey_process_16(void* sndbuffer, unsigned sndn)
{
    unsigned short i;
    int nsam = sndn;
	float *buffer = (float *) sndbuffer;	//f0 from int16 to float
	...
	while(nsam >= (int) num_cur_pokeys)
    {
        ...
        // * (65535.0 / MAX_SAMPLE / 4 * M_PI * 0.95) + 0.5 + 0.5 * rand() / RAND_MAX - 0.25);
		buffer[0] = (generate_sample(pokey_states) / 122.0)
         + 0.5 + (0.5 * (rand() / RAND_MAX)) - 0.25;		//f0 scaled to 0-1
#endif
        for(i=1; i<num_cur_pokeys; i++)
        {
            //buffer[i] = (int16)floor((generate_sample(pokey_states + i) - MAX_SAMPLE / 2.0)
            // * (65535.0 / MAX_SAMPLE / 4 * M_PI * 0.95) + 0.5 + 0.5 * rand() / RAND_MAX - 0.25);
			buffer[i] = (generate_sample(pokey_states + i) / 122.0)
             + 0.5 + (0.5 * (rand() / RAND_MAX)) - 0.25;	//f0 scaled to 0-1 (but unused as only 1 pokey)
        }
        buffer += num_cur_pokeys;
        nsam -= num_cur_pokeys;
    }
