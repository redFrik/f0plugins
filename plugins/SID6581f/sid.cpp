...
float filterTable[2048];									//f0 -changed these
float bandPassParam[2048];
...
/*INLINE*/ void syncEm(SID6581f *This)					//f0 -removed inline
{
...
void filterTableInit(void)
{
	...
	//filterTable = auto_malloc(sizeof(*filterTable) * 0x800);			//f0 -commented out and moved to top
	...
	for ( rk = 0; rk < 0x800; rk++ )
	{
		filterTable[uk] = (((exp(rk/0x800*log(400.0))/60.0)+0.05)
			*filterRefFreq) / 44100;									//f0 -have to fix this??? was Machine->sample_rate
	...
	{
		bandPassParam[uk] = (yTmp*filterRefFreq) / 44100;				//f0 -have to fix this??? was Machine->sample_rate
		yTmp += yAdd;
		uk++;
	}
...
//	sidInitMixerEngine();				//f0 -commented out and moved to ctor
//	filterTableInit();					//f0 -commented out and moved to ctor

//	sidInitWaveformTables(This->type);	//f0 -commented out and moved to ctor

//	enveEmuInit(This->PCMfreq, TRUE);	//f0 -commented out and moved to ctor

//	MixerInit(0);						//f0 -commented out and moved to ctor
...
void sid6581_port_w (SID6581f *This, int offset, int data)
{
	...
	switch (offset)
	{
		...
		case 0x18:
//			stream_update(This->mixer_channel);					//f0 -commented out???
