/*
	...
	--
	Nes - APU Chip Sound Simulator for SC
...
**
** NES APU emulation
** $Id: nes_apu.c,v 1.2 2003/04/09 14:50:32 ben Exp $	(severely hacked for the sc version /f0)
*/
...
//#include "log.h"		//f0
#include "nes_apu.h"
//#include "nes6502.h"	//f0
#define  ASSERT(expr)	//f0
...
static int apu_enqueue(apudata_t *d)
{
   ...
   if (APU_QEMPTY()) {
      /*log_*/printf("apu: queue overflow\n");      //f0
...
PluginLoad(InterfaceTable *inTable) {	//f0
	ft= inTable;
	DefineSimpleUnit(Nes2);
}

void Nes2_Ctor(Nes2 *unit) {			//f0
	if(INRATE(0)==calc_FullRate) {
		SETCALC(Nes2_next_a);
	} else {
		SETCALC(Nes2_next_k);
	}
	apu_create(unit, SAMPLERATE, 60, 16, FALSE);
	apu_regwrite(unit, APU_SMASK, 0x2);
	unit->prevtrig= 0.f;
	Nes2_next_k(unit, 1);
}

void Nes2_next_k(Nes2 *unit, int inNumSamples) {	//f0
	float *out= OUT(0);
	float curtrig= IN0(0);
	if(curtrig>0.f && unit->prevtrig<=0.f) {
		apu_regwrite(unit, APU_SMASK, IN0(19));		//unit, addr, val
		apu_regwrite(unit, APU_WRA0, IN0(1));
		apu_regwrite(unit, APU_WRA1, IN0(2));
		apu_regwrite(unit, APU_WRA2, IN0(3));
		apu_regwrite(unit, APU_WRA3, IN0(4));
		apu_regwrite(unit, APU_WRB0, IN0(5));
		apu_regwrite(unit, APU_WRB1, IN0(6));
		apu_regwrite(unit, APU_WRB2, IN0(7));
		apu_regwrite(unit, APU_WRB3, IN0(8));
		apu_regwrite(unit, APU_WRC0, IN0(9));
		apu_regwrite(unit, APU_WRC2, IN0(10));
		apu_regwrite(unit, APU_WRC3, IN0(11));
		apu_regwrite(unit, APU_WRD0, IN0(12));
		apu_regwrite(unit, APU_WRD2, IN0(13));
		apu_regwrite(unit, APU_WRD3, IN0(14));
		apu_regwrite(unit, APU_WRE0, IN0(15));
		apu_regwrite(unit, APU_WRE1, IN0(16));
		apu_regwrite(unit, APU_WRE2, IN0(17));
		apu_regwrite(unit, APU_WRE3, IN0(18));
	}
	apu_process(unit, out, inNumSamples);			//unit, buffer, n
	unit->prevtrig= curtrig;
}

void Nes2_next_a(Nes2 *unit, int inNumSamples) {	//f0
	float *out= OUT(0);
	float *trig= IN(0);
   ...
   if (chan->dma_length)
   {
      ...
      while (chan->phaseacc < 0)
      {
         ...
         if (7 == delta_bit)
         {
            ...
            chan->cur_byte = chan->cur_byte + 1;		//f0 changed here
...
void apu_regwrite(Nes2 *apu, uint32 address, uint8 value)	//f0 nonstatic and added apu
{  
   int chan;
...
void apu_write(uint32 address, uint8 value)
{
   ...
   case 0x4010: case 0x4011: case 0x4012: case 0x4013:
      d.timestamp = 0;//nes6502_getcycles(FALSE);	//f0hack
      d.address = address;
      d.value = value;
      apu_enqueue(&d);
      break;
...
void apu_process(Nes2 *apu, float *buffer, int num_samples)	//f0 changed to float and added apu
{
   apudata_t *d;
   ...
   while (num_samples--)
   {
      ...
      {
         d = apu_dequeue();
         apu_regwrite(apu, d->address, d->value);	//f0
      }
      ...
      if (16 == apu->sample_bits)
		 ...
		 *buffer++ = (float)accum/32767.0;
      else
         *(uint8 *)(buffer)++ = (accum >> 8) ^ 0x80;
   }

   /* resync cycle counter */
   apu->elapsed_cycles = 0;//nes6502_getcycles(FALSE);	//f0hack
}
...
void apu_reset(Nes2 *apu)	//f0 added apu
{
   uint32 address;

   ASSERT(apu);
	
   apu->rectangle[0].sweep_phase= 0;		//f0 added these bug fixes
   apu->rectangle[1].sweep_phase= 0;
   apu->rectangle[0].env_phase= 0;
   apu->rectangle[1].env_phase= 0;
   apu->rectangle[0].env_vol= 0;
   apu->rectangle[1].env_vol= 0;
   apu->noise.env_phase= 0;
   apu->noise.env_delay= 0;
   apu->noise.env_vol= 0;
   apu->noise.fixed_envelope= 0;
   apu->noise.holdnote= 0;

   apu->elapsed_cycles = 0;
   memset(&apu->queue, 0, APUQUEUE_SIZE * sizeof(apudata_t));	//f0???
   apu->q_head = 0;
   apu->q_tail = 0;

   /* use to avoid bugs =) */
   for (address = 0x4000; address <= 0x4013; address++)
      apu_regwrite(apu, address, 0);

#ifdef NSF_PLAYER
   apu_regwrite(apu, 0x400C, 0x10); /* silence noise channel on NSF start */
   apu_regwrite(apu, 0x4015, 0x0F);
#else
   apu_regwrite(apu, 0x4015, 0);
#endif /* NSF_PLAYER */
...
void apu_create(Nes2 *temp_apu, int sample_rate, int refresh_rate, int sample_bits, boolean stereo)	//f0
{
   //apu_t *temp_apu;	//f0
/*    int channel; */

   //temp_apu = malloc(sizeof(apu_t));	//f0
   //if (NULL == temp_apu)				//f0
   //   return NULL;					//f0
   /* $$$ ben : safety net, in case we forgot to init something */
   //memset(temp_apu,0,sizeof(apu_t));	//f0
