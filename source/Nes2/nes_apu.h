...
** $Id: nes_apu.h,v 1.2 2003/04/09 14:50:32 ben Exp $		modified for sc 070227 /f0
*/
...
typedef struct Nes2 : public Unit	//f0
{
  ...
  int mix_enable; /* $$$ben : should improve emulation */
   ...
   void (*process)(Nes2 *unit, float *buffer, int num_samples);		//f0 changed to float
...
/* Function prototypes */
//extern apu_t *apu_create(int sample_rate, int refresh_rate, int sample_bits, boolean stereo);	//f0
extern void apu_create(Nes2 *unit, int sample_rate, int refresh_rate, int sample_bits, boolean stereo);	//f0
extern void apu_destroy(apu_t *apu);
extern int apu_setext(apu_t *apu, apuext_t *ext);
extern int apu_setfilter(int filter_type);
extern void apu_process(Nes2 *unit, float *buffer, int num_samples);	//f0 changed to float and added unit
extern void apu_reset(Nes2 *unit);										//f0
extern int apu_setchan(int chan, boolean enabled);
extern int32 apu_getcyclerate(void);
extern apu_t *apu_getcontext(void);

extern void apu_regwrite(Nes2 *unit, uint32 address, uint8 value);		//f0
